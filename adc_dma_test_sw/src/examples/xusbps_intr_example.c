...
static int usb_init(XScuGic *IntcInstancePtr, XUsbPs *UsbInstancePtr,
...
static void MEF_input(void);
static void MEF_output(XUsbPs *UsbInstancePtr);
...
#define OUTPUT_WORDS        4096 * 16
#define OUTPUT_BUFFER_SIZE  (OUTPUT_WORDS * 4) // u32
#define CHUNK_BYTES 		(16 * 512)

static volatile u8  BufferInput[INPUT_BUFFER_SIZE];
static volatile u32 InputWrIdx = 0;
static volatile u32 InputRdIdx = 0;

static u32 BufferOutput[OUTPUT_WORDS];
static volatile int streaming_enabled = 0;
...
int main2(void)
{
	...
	Status = usb_init(&IntcInstance, &UsbInstance, XPAR_XUSBPS_0_DEVICE_ID, XPAR_XUSBPS_0_INTR);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}

	/* Inicializar buffer de salida */
	for (u32 i = 0; i < OUTPUT_WORDS; i++) {
	    BufferOutput[i] = i + 1;
	}

	while (1) {
	    MEF_input();
	    MEF_output(&UsbInstance);
	}

	return XST_SUCCESS;
}

static u32 session_marker = 0;
...
void MEF_input(void)
{
    ...
    while (InputRdIdx != InputWrIdx) {
        char c = BufferInput[InputRdIdx++ % INPUT_BUFFER_SIZE];
        ...
        if (idx == 6 ) {
        	if (memcmp(cmd, "ASTART", 6) == 0){
        		Xil_DCacheFlushRange((UINTPTR)&session_marker, sizeof(u32));
        		XUsbPs_EpBufferSend(&UsbInstance, 1, (u8*)&session_marker, sizeof(u32));
        		out_offset = 0;
				streaming_enabled = 1;
				idx = 0;
        	}
        } else if (idx == 5) {
        	if (memcmp(cmd, "ASTOP", 5) == 0){
				streaming_enabled = 0;
				idx = 0;
        	}
...
void MEF_output(XUsbPs *UsbInstancePtr)
{
    ...
    u32 remaining = OUTPUT_BUFFER_SIZE - out_offset;
	u32 send_bytes = (remaining >= CHUNK_BYTES) ?
					  CHUNK_BYTES : remaining;

	Xil_DCacheFlushRange(
	    (UINTPTR)((u8*)BufferOutput + out_offset),
	    send_bytes
	);

	XUsbPs_EpBufferSend(&UsbInstance,
						1,
						(u8 *)BufferOutput + out_offset,
						send_bytes);

	out_offset += send_bytes;

	if (out_offset >= OUTPUT_BUFFER_SIZE) {
		out_offset = 0;   // o detener streaming
	}
}
                                  ...
                                  u8 EventType, void *Data)
{
    ...
    case XUSBPS_EP_EVENT_DATA_RX:
        if (XUsbPs_EpBufferReceive(InstancePtr, EpNum,
                                   &BufferPtr, &BufferLen, &Handle) == XST_SUCCESS) {
...
//
//	case XUSBPS_EP_EVENT_DATA_TX:
//		// Los datos fueron completamente consumidos por el HOST
//		tx_is_ready = 1;
//		break;
//
//	default:
