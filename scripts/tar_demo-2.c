...
void serial_SendCommand(char c)
{
    char cc = (char)0x25;
    write(serial_fd, &cc, 1); // header
    write(serial_fd, &c, 1);  // comando
}
void serial_SendCommand_(char c, uint32_t p)
{
    char pp = 0;
    serial_SendCommand(c);

    // Enviar el parámetro como 4 bytes (big endian o little endian según necesites)
    pp = (p >> 24) & 0xFF;
    write(serial_fd, &pp, 1); // parametro
...
int main()
{
    ...
    serial_SendCommand_((char)0xA0, to_hist(to_cad(1000), to_cad(3000)));

    serial_SendCommand_((char)0xB0, to_hist(to_cad(1000), to_cad(3000)));

    usleep(to_us(1));

    serial_SendCommand((char)0x01); // satrt

    usleep(to_us(10));

    serial_SendCommand((char)0x02); // stop

    usleep(to_us(3)); // pausa para terminar de recibir todo lo que falte

    keepReceiving = 0;
    mefSerialToBin(0, 0, 0);
    mefSerialToBin(0, 0, 0);
